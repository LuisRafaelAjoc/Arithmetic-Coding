...
with open("examples/text.txt", "rb") as img:
    raw_bytes = img.read()
...
with open('decoded_file.txt', 'wb') as decode: # Open image file to save.
...
with open("text_encoded.bin", "wb") as f:
...
# Sizes
original_size = os.stat("examples/text.txt").st_size
print(f"Size of original file: {original_size}")
compressed_size = os.stat("text_encoded.bin").st_size
