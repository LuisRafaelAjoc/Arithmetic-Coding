...
with open("examples/image.tiff", "rb") as img:
    raw_bytes = img.read()
...
with open('decoded_image.tiff', 'wb') as decode: # Open image file to save.
...
# Sizes
original_size = os.stat("examples/image.tiff").st_size
print(f"Size of original image: {original_size}")
compressed_size = os.stat("image_encoded.bin").st_size
